	...
	async fetch(request, _env, _ctx) {
		...
		if (request.method === "POST" && url.pathname === "/api/deploy") {
			try {
				...
				const workerName = `q-panel-${uniqueSuffix}`;
				...
				formData.append(
					"q.js",
					new Blob([zeusCode], { type: "application/javascript+module" }),
					"q.js",
				);
		...
		if (request.method === "POST" && url.pathname === "/api/list-panels") {
			try {
				...
				const subData = await subRes.json();
				const devSub =
					subData.success && subData.result && subData.result.subdomain
						? subData.result.subdomain
						: "";
				...
				for (const script of scriptsData.result) {
					if (script.id.startsWith("q-panel") || script.id.startsWith("ez-")) {
				...
				try {
					const ghRes = await fetch(
						"https://raw.githubusercontent.com/QMahyar/q-panel/main/q.js?t=" +
							Date.now(),
					);
					if (ghRes.ok) {
				...
				return new Response(
					JSON.stringify({ success: true, panels, latestVersion, devSub }),
					{
						headers: { "Content-Type": "application/json" },
					},
				);
		...
		if (request.method === "POST" && url.pathname === "/api/delete-panel") {
			try {
				...
				if (!deleteData.success) {
					const cfError =
						deleteData.errors && deleteData.errors.length > 0
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center justify-center p-4">
    <div id="mainCard" class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-3xl shadow-2xl p-8 relative overflow-hidden z-10">
        ...
        <div class="absolute -right-12 -bottom-12 w-40 h-40 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none"></div>
            ...
            <h2 class="text-2xl font-black text-gray-900 dark:text-white mb-2">Q Panel Deployer</h2>
...
<div class="flex flex-col gap-4 mt-6 z-10">
    <div class="flex items-center gap-4 justify-center">
        <a href="https://github.com/QMahyar/q-panel" target="_blank" class="flex items-center gap-2 px-4 py-2 border border-gray-700 text-gray-500 bg-gray-900/20 hover:bg-gray-900/40 rounded-full shadow-sm hover:shadow-md transition text-sm font-bold group">
        ...
        <a href="https://t.me/Q_PANEL_BOT" target="_blank" class="flex items-center gap-2 px-4 py-2 border border-sky-700 text-sky-500 bg-sky-900/20 hover:bg-sky-900/40 rounded-full shadow-sm hover:shadow-md transition text-sm font-bold group">
            <svg class="w-5 h-5 group-hover:scale-110 transition" viewBox="0 0 24 24" fill="currentColor">
    ...
    <div class="flex items-center gap-4 justify-center">
        <a href="#" class="flex items-center gap-2 px-4 py-2 border border-amber-700 text-amber-500 bg-amber-900/20 hover:bg-amber-900/40 rounded-full shadow-sm hover:shadow-md transition text-sm font-bold group">
