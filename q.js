...
const Router = {
	...
	async handleApi(request, url, env, ctx) {
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			const currentToken = env.CF_API_TOKEN || body.cf_token;
			...
			try {
				...
				const githubRes = await fetch(
					"https://raw.githubusercontent.com/QMahyar/q-panel/refs/heads/main/q.js?t=" +
						Date.now() +
						Math.random(),
					{
						headers: {
				...
				const errorMsg =
					err.message +
					" | در صورت عدم موفقیت، از طریق لینک زیر آپدیت کنید: https://github.com/QMahyar/q-panel";
		...
		if (url.pathname === "/api/restart-core" && request.method === "POST") {
			...
			try {
				...
				const bindingsData = await bindingsRes.json();
				if (!bindingsData.success)
					throw new Error("عدم دسترسی به تنظیمات ورکر");
				...
				const deployData = await deployRes.json();
				if (!deployData.success)
					throw new Error("خطا در اعمال ری‌استارت در کلودفلر");
			...
			if (isUserAction) {
				...
				if (request.method === "PUT") {
					...
					if (body.toggle_only !== undefined) {
						...
						await env.DB.prepare(
							"UPDATE users SET username = ?, limit_gb = ?, expiry_days = ?, limit_req = ?, ips = ?, tls = ?, port = ?, fingerprint = ?, max_connections = ?, ip_limit = ?, block_porn = ?, block_ads = ?, frag_len = ?, frag_int = ?, user_proxy_iata = ?, user_socks5 = ?, user_proxy_ip = ? WHERE username = ?",
						)
							.bind(
				...
				if (request.method === "POST") {
					...
					try {
						await env.DB.prepare(
							"INSERT INTO users (username, uuid, limit_gb, expiry_days, limit_req, ips, connection_type, tls, port, fingerprint, max_connections, ip_limit, used_gb, used_req, created_at, is_active, block_porn, block_ads, frag_len, frag_int, user_proxy_iata, user_socks5, user_proxy_ip) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
						)
							.bind(
								username,
								finalUuid,
...
const SubscriptionService = {
	async generateText(user, host) {
		...
		if (user.limit_gb) {
			const rem = user.limit_gb - (user.used_gb || 0);
		...
		if (user.limit_req) {
			const rem = user.limit_req - (user.used_req || 0);
		...
		links.push(
			atob("dmxlc3M6Ly8=") +
				...
				"@" +
				host +
				":80?path=%2Fq&security=none&encryption=none&host=" +
				host +
				"&fp=" +
				fp +
				"&type=ws#" +
...
async function handleVLESS(env, storedData = null, ctx = null, request = null) {
	...
	function addBytes(bytes) {
		...
		const current = GLOBAL_TRAFFIC_CACHE.get(username) || 0;
		...
		const lastDbWrite = GLOBAL_LAST_DB_WRITE.get(username) || 0;
		const now = Date.now();
		...
		if (
			...
			const toCommit = GLOBAL_TRAFFIC_CACHE.get(username) || 0;
			const toCommitReq = USER_REQ_CACHE.get(username) || 0;
			...
			const removeIpTask = async () => {
				try {
					...
					if (user) {
						...
						const activeIps = JSON.parse(user.active_ips || "{}");
	...
	const processWsMessage = async (chunk) => {
		...
		if (!isHeaderParsed) {
			...
			if (clientIP && clientIP !== "unknown") {
				console.log(
					`[VLESS Handshake] User: ${user.username}, clientIP: ${clientIP}, active_ips in DB: ${user.active_ips}`,
				);
				let activeIps = {};
				try {
					activeIps = JSON.parse(user.active_ips || "{}");
				...
				if (!activeIps[clientIP]) {
					...
					console.log(
						`[VLESS Handshake] Non-expired active IPs: ${JSON.stringify(activeIps)}, count: ${sortedIps.length}, limit: ${user.ip_limit}`,
					);
					if (
						user.ip_limit &&
						user.ip_limit > 0 &&
						...
						console.log(
							`[VLESS Handshake] BLOCKED user ${user.username} because sortedIps.length (${sortedIps.length}) >= limit (${user.ip_limit})`,
						);
						serverSock.close();
						return;
					}
					...
					console.log(
						`[VLESS Handshake] Reconnected from same IP: ${clientIP}, count: ${activeIps[clientIP].count}`,
					);
				}
				try {
					await env.DB.prepare(
						...
						.run();
					console.log(
						`[VLESS Handshake] Successfully updated active_ips to: ${JSON.stringify(activeIps)}`,
					);
				} catch (e) {
...
async function forwardVlessUDP(
	udpChunk,
	webSocket,
	respHeader,
	onBytes,
...
async function connectProxy(proxyStr, destAddr, destPort, initialData) {
	...
	const cleanStr = normalized.replace(
		/^(socks4|socks5|socks|http|https):\/\//i,
		"",
	);
...
async function connectSocks5(socksStr, destAddr, destPort, initialData) {
	...
	try {
		...
		if (method === 0x02) {
			...
			const authRes = await reader.read();
		...
		const connRes = await reader.read();
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                ...
                <div class="flex items-center gap-3 bg-gray-100 dark:bg-zinc-800/60 px-3 py-1.5 rounded-full border border-gray-200 dark:border-zinc-800/80 shadow-sm flex-shrink-0 w-fit">
                    <a href="https://github.com/QMahyar/q-panel" target="_blank" rel="noopener noreferrer" class="text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="GitHub">
                ...
                <div class="space-y-2.5">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="input-name" placeholder="Q_PANEL" maxlength="32" class="w-full pl-3 pr-9 py-1.5 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-xs font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition" required>
                        </div>
                    </div>
                        ...
                        <div>
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 border-b border-gray-100 dark:border-zinc-800/30 pb-3">
	                            ...
	                            <div class="grid grid-cols-2 sm:flex items-center gap-2 w-full sm:w-auto">
	                            	...
	                            	<a href="https://github.com/QMahyar/q-panel-relay" target="_blank" class="text-[11px] bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-2 sm:py-0.5 rounded border border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition font-black shadow-sm text-center whitespace-nowrap">ساخت پروکسی شخصی</a>
	                            </div>
                            </div>
        ...
        function renderUsersUI(data) {
            try {
                ...
                if (cfRequests >= 90000) {
                    ...
                    const today = new Date().toISOString().split('T')[0];
                    if (localStorage.getItem('q_usage_warned_date') !== today) {
                        openUsageWarning();
                    }
                } else {
                    if (reqCard) {
...
function closePathWarning() {
    ...
    card.classList.remove('opacity-100', 'scale-100');
    card.classList.add('opacity-0', 'scale-95');
    localStorage.setItem('q_path_warned_' + CURRENT_VERSION, 'true');
}
function closeUsageWarning() {
    ...
    card.classList.add('opacity-0', 'scale-95');
    const today = new Date().toISOString().split('T')[0];
    localStorage.setItem('q_usage_warned_date', today);
}
...
function getVlessLink(username) {
            ...
            links.push('vle' + 'ss://' + (user.uuid || '') + '@0.0.0.0:1?encryption=none&security=none&type=ws&host=' + host + '&path=%2Fq#' + encodeURIComponent(m1));
            ips.forEach((ip) => {
                ports.forEach((portStr) => {
                    const isTlsPort = tlsPorts.includes(portStr);
                    const tlsVal = isTlsPort ? 'tls' : 'none';
                    const remark = user.username + ' | \u200E' + ip + ' | \u200E' + portStr;
                    links.push('vle' + 'ss://' + (user.uuid || '') + '@' + ip + ':' + portStr + '?path=%2Fq&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + userFrag + '#' + encodeURIComponent(remark));
                });
            });
            return links.join('\\n');
        }
    ...
    try {
        ...
        async function exportUsersBackup() {
            ...
            try {
                ...
                downloadAnchor.setAttribute("download", "q_full_backup_" + dateStr + ".json");
...
const UPDATE_FIX = "constsCURRENT_VERSION='d.d.d'";
		async function checkForUpdates(isManual = false) {
            try {
                ...
                const res = await fetch('https://raw.githubusercontent.com/QMahyar/q-panel/refs/heads/main/q.js?t=' + Date.now());
...
async function fetchIpsList() {
    try {
        const response = await fetch('https://raw.githubusercontent.com/QMahyar/q-panel/refs/heads/main/ips.txt');
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            if (localStorage.getItem('q_path_warned_' + CURRENT_VERSION) !== 'true') {
                const modal = document.getElementById('path-warning-modal');
                const card = modal.querySelector('div');
                modal.classList.remove('opacity-0', 'pointer-events-none');
                modal.classList.add('opacity-100', 'pointer-events-auto');
            ...
            window.changeRefreshRate = function(val) {
                ...
                localStorage.setItem('q_refresh_rate', ms);
                window.startRefreshInterval(ms);
                showToast('نرخ رفرش پنل تغییر کرد');
            };

            const savedRate = localStorage.getItem('q_refresh_rate');
...
function toggleProxySelectorModal(show) {
    ...
    if (show) {
		...
		async function loadVipCountries() {
			...
			try {
				const res = await fetch('https://api.github.com/repos/QMahyar/q-panel/contents/proxy/proxy_vip');
		...
		async function loadVipProxy() {
			...
			try {
				const url = 'https://raw.githubusercontent.com/QMahyar/q-panel/refs/heads/main/proxy/proxy_vip/' + country + '.txt?t=' + Date.now();
...
async function fetchAndLoadProxy() {
    ...
    try {
		const sources = [
			...
			{ url: 'https://raw.githubusercontent.com/QMahyar/q-panel/refs/heads/main/proxy/' + country.toUpperCase() + '.txt', prefix: '' },
...
<div class="flex flex-col gap-4 mt-6 z-10">
    <div class="flex flex-wrap items-center gap-3 sm:gap-4 justify-center">
        <a href="https://github.com/QMahyar/q-panel" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-gray-700 dark:text-zinc-300 hover:text-black dark:hover:text-white group">
        ...
        function getVlessLink() {
            ...
            ips.forEach(function(ip, ipIndex) {
                ports.forEach(function(portStr) {
                    ...
                    links.push('vle' + 'ss://' + (u.uuid || '') + '@' + ip + ':' + portStr + '?path=%2Fq&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + userFrag + '#' + encodeURIComponent(remark));
                });
            });
            return links.join('\\n');
        }
